...
import com.aktimetrix.core.util.LogUtil;
...
public class DefaultRegistry implements Registry {
    ...
    private static Logger log = LoggerFactory.getLogger(DefaultRegistry.class);
            ...
            attributes.forEach((key, value) -> {
                ...
                if (attributeMap.containsKey(key)) {
                    ...
                    try {
                        ...
                        LogUtil.warn(log, "Processing (%s,%s) failed due to Exception %s. So ignoring the attribute value", key, value, e);
                    }
                } else {
                    LogUtil.warn(log, "Seems no Attribute Provider for Attribute(%s), Please check the configuration...", key);
                }
            });

        } catch (UnknownNameException e) {
            LogUtil.info(log, "Seems no Attribute Providers are present. So Attribute conversion failed (%s) ..", e.getMessage());
            return attributes;
        }
        return retVal;
    }
